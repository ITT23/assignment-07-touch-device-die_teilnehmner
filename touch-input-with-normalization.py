...
def main():
    ...
    while(True):
        ...
        contours = []
        #Threshold Tab
        ret, thresh_tab = cv2.threshold(img_gray, THRESHOLD_TAB, 255, cv2.THRESH_BINARY)
        #thresh = cv2.adaptiveThreshold(img_gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 11, 2)
        contours, hierarchy = cv2.findContours(thresh_tab, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
